...
const recursor = (query, isDone, makeNext, makeFirst) => new Promise((resolve, reject) => {
  let all = []
  const rec = (page) => {
    const now = Date.now()
    return query(page)
      .then((x) => {
        ...
        if (isDone(x)) { return resolve({ headers: x.headers, body: all }) }
        setTimeout((p) => rec(p), waitFor(Date.now() - now, x), makeNext(page))
      })
  }
  return rec(makeFirst())
})
...
exports.findQcGroups = () => recursor(findQcGroupsPage, (x) => !x.body.length, (page) => page + 1, () => 0)
